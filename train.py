...
if __name__ == '__main__':
    ...
    4. 添加pred循环
    """
    # model.set_state_dict(paddle.load("Ref_valid_epoch15_loss[34.833633]"))
    # MAX_EPOCH = 20 #200
    # model.train()
    ...
    #         paddle.save(model.state_dict(), 'yolo_epoch{}_loss{}'.format(epoch, loss.numpy()))
    ...
    for i, data in enumerate(valid_loader()):
        ...
        for j in range(len(result)):
            ...
            # get labels, scores and boxes from result_j
            if len(result_j) < MAX_NUM:
                pad_zeros = np.zeros((MAX_NUM - len(result_j)))
                pred_score = np.concatenate((result_j[:,1], pad_zeros), axis=0)
                pred_label = np.concatenate((result_j[:,0], pad_zeros), axis=0)
                pad_zeros_2D = np.zeros((MAX_NUM - len(result_j), 4))
                pred_box = np.concatenate((result_j[:,2:], pad_zeros_2D), axis=0)
            else:
                pred_label = result_j[:MAX_NUM, 0]
                pred_score = result_j[:MAX_NUM, 1]
                pred_box = result_j[:MAX_NUM, 2:]
        ...
        for _m in metrics:
            _m.update(gt_boxes, gt_labels, img_scale, pred_boxes, pred_labels[0], pred_scores)
